...
from mysql.models               import (CreatureSlots,
                                        Cosmetic,
                                        Item)
...
def fn_slots_add(creature):
    ...
    try:
        ...
        session.rollback()
        logger.error(f'{h} Slots Query KO [{e}]')
        return None
    else:
        if slots:
            logger.trace(f'{h} Slots Query OK')
            return slots
        else:
            logger.warning(f'{h} Slots Query KO - NotFound')
            return None
    finally:
        session.close()
...
def fn_slots_set_one(creature, slot, itemid):
    ...
    try:
        ...
        if itemid is None:
            ...
            setattr(slots, slot, item.id)

            item.bound     = True  # Now item is bound to PC
